        ...
        <div>
            ...
            @if ($eval->evalAdq === null)
                <form method="POST" action="/admin/evaladq">
                    ...
                    <button type="button" class="bg-laravel text-white rounded-lg px-4 py-2 border"
                        data-modal-target="popup-modal{{ $eval->id }}"
                        data-modal-toggle="popup-modal{{ $eval->id }}">Asignar</button>
                    <div id="popup-modal{{ $eval->id }}" tabindex="-1"
                        class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                        <div class="relative w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                <button type="button"
                                    class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white"
                                    data-modal-hide="popup-modal{{ $eval->id }}">
                                ...
                                <div class="p-6 text-center">
                                    ...
                                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                        ...
                                        <div class="flex gap-4 text-center justify-center m-4">
                                            <button data-modal-hide="popup-modal{{ $eval->id }}" type="submit"
                                                class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                                Sí, estoy seguro
                                            </button>
                                            <button data-modal-hide="popup-modal{{ $eval->id }}" type="button"
                ...
                <div class="text-center">
                    ...
                    @php
                        $evaluador = DB::table('users')
                            ->where('id', $eval->evalAdq->id_revisor)
                            ->first();
                    @endphp
                    ...
                    @php
                        $hayevals = DB::table('evals_adquiridas')
                            ->where('id_evaladq', $eval->evalAdq->id)
                            ->first();
                    @endphp
                    ...
                    @if ($hayevals !== null)
                        <div class="p-4 shadow border rounded-lg my-4">
                            ...
                            <a href="/admin/evaladq/{{ $eval->evalAdq->id }}"><button
                                    class="bg-laravel text-white p-4 m-4 rounded-lg">Revisar</button></a>
                        </div>
                    @endif
