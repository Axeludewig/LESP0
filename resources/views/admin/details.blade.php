                    ...
                    @foreach ($participantes as $participante)
                        <li>{{$num . " - "}}{{ $participante->nombre_participante }} &nbsp; <form method="POST" action="/admin/destroyparticipante/{{$participante->id}}">
                            <div class="mb-2">
                                @csrf
                                @method('DELETE')
                                <button  data-modal-target="popup-modal{{$participante->id}}" data-modal-toggle="popup-modal{{$participante->id}}" type="button"  class="text-red-500"><i class="fa-solid fa-trash"></i> Eliminar</button>
                            </div>
            
                            <div id="popup-modal{{$participante->id}}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                <div class="relative w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-hide="popup-modal{{$participante->id}}">
                                        ...
                                        <div class="p-6 text-center">
                                            ...
                                            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">¿Estás seguro que deseas eliminar el participante: {{ $participante->nombre_participante }}?</h3>
                                            <button data-modal-hide="popup-modal{{$participante->id}}" type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                                Sí, estoy seguro
                                            </button>
                                            <button data-modal-hide="popup-modal{{$participante->id}}" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No, cancelar</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
